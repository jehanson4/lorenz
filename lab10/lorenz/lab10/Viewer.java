...
public class Viewer {
	...
	public interface Timeseries {
		...
		public abstract void setMaxHistory(int maxHistory);
	}
	...
	private final class TimeseriesImpl implements Timeseries, DataSourceListener {
		...
		public void setMaxHistory(int maxHistory) {
			if (!(maxHistory > 0))
				throw new IllegalArgumentException("Bad value maxHistory=" + maxHistory + " -- must be > 0");
		...
		public void dataPointGenerated(DataSourceEvent e) {
			synchronized (eventsToDraw) {
				...
				while (eventsToDraw.size() > maxHistory)
	...
	private class CListener implements ControlListener, PaintListener {
		...
		public void paintControl(PaintEvent e) {
			...
			for (ViewDecorator d : decorators) {
				d.draw(gc, clientArea, transform);
			}
	...
	public Viewer() {
		...
		this.dataBounds = new DataBox(DataPoint.ZERO, 2.0);
		this.canvas = null;
		this.display = null;
		this.clearRequested = false;
		this.redrawRequested = false;
		this.decorators = new ArrayList<ViewDecorator>();
	...
	public void addViewDecorator(ViewDecorator d) {
		if (d != null)
			decorators.add(d);
	}

	public void removeViewDecorator(ViewDecorator d) {
		decorators.remove(d);
	}
