...
public class RungeKutta4_3D extends DataSource {
	...
	public RungeKutta4_3D(ODESystem_3D odeSystem) {
		this("", odeSystem, (odeSystem == null) ? DataPoint.ZERO : odeSystem
				.getInitialStateHint());
	}

	public RungeKutta4_3D(String name, ODESystem_3D odeSystem) {
		this(name, odeSystem, (odeSystem == null) ? DataPoint.ZERO : odeSystem
				.getInitialStateHint());
	}

	public RungeKutta4_3D(String name, ODESystem_3D odeSystem, DataPoint initialState) {
		super(name);
		if (odeSystem == null)
		...
		this.odeBounds = odeSystem.getDataBounds();
	...
	public DataBox getDataBoundsHint() {
		return odeSystem.getDataBounds();
	}
	...
	protected void doStep() {
		...
		// temp variables; preallocate?
		double deltaT_2 = timeStep * 0.5;
		double deltaT_6 = timeStep / 6.0;
		double[] tmpP = new double[3]; // temp variable: position estimates
		double[] tmpPDot1 = new double[3]; // temp variable: deriv estimates
		double[] tmpPDot2 = new double[3]; // temp variable: deriv estimates
		double[] tmpPDot3 = new double[3]; // temp variable: deriv estimates

		final double nextT_2 = currT + deltaT_2; // half-way to nextT
		final double nextT = currT + timeStep;

		// 1. Take derivatives at currP, use them to calculate first estimate of
		// "nextP_2" (i.e., position at nextT_2)
		odeSystem.takeDerivatives(currT, currP, tmpPDot1);
		for (int i = 0; i < 3; i++)
			tmpP[i] = currP[i] + deltaT_2 * tmpPDot1[i];

		// 2. take derivatives at our first estimate of nextP_2, use them to
		// calculate second estimate of nextP_2
		odeSystem.takeDerivatives(nextT_2, tmpP, tmpPDot2);
		for (int i = 0; i < 3; i++)
			tmpP[i] = currP[i] + deltaT_2 * tmpPDot2[i];

		// 3. take derivatives at second estimate of nextP_2, use them to
		// calculate first estimate of nextP. Also, fiddle with tmpPDot3 in
		// prep for step 4.
		...
		// 4. take derivatives at first estimate of nextP. Combine all
		// estimates with proper weighting to get second and final estimate of
		// nextP.
		...
		currP[0] = odeBounds.applyXBounds(currP[0]);
		currP[1] = odeBounds.applyYBounds(currP[1]);
		currP[2] = odeBounds.applyZBounds(currP[2]);
		currT = nextT;

		if (logger.isLoggable(Level.FINER))
			logger.logp(Level.FINER, clsName, mtdName, "Exiting. currP="
					+ getCurrentPoint());
	}
